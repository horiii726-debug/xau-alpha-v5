...
def triple_barrier_labels(
    ...
    n_bars = len(open_)
    for i in range(n):
        e = int(entry_bar_idx[i])
        d = direction[i]
        if e >= n_bars or e < 0:
            continue
        s = sigma[e - 1] if e > 0 else np.nan  # sigma known as of entry bar's open (prior bar close, causal)
        ...
        for t in range(e, end):
            ...
            if touched_tp and touched_sl:
                # both touched intrabar -- conservative assumption: SL first (can't know order from OHLC alone)
                hit = -1
                exit_bar = t
                exit_price = sl_price
                break
    ...
    return BarrierResult(outcome=outcome, ret=ret, bars_held=bars_held, entry_idx=entry_bar_idx)
