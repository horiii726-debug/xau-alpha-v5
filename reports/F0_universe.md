...
- N observasi PnL selaras (irisan timestamp XAUUSD & XAGUSD): 332,051
- **Korelasi PnL strategi baseline XAUUSD-XAGUSD (rho_PnL terukur): 0.4556**
- Eigenvalues matriks korelasi 2x2: [0.5444, 1.4556]
- **K_eff (metode eigenvalue, WAJIB dipakai resmi): 1.6562**
- K_eff (metode equicorrelated, perencanaan saja): 1.3740

## Batas matematis (dinyatakan SEBELUM run, lihat PREREGISTRATION.md)

Untuk K=2 instrumen, `K_eff_eigen = 2 / (1 + rho^2)`, yang **terikat ke rentang (1, 2]** untuk SEMUA nilai rho yang mungkin (termasuk rho negatif). Tidak ada nilai korelasi PnL yang bisa membuat K_eff panel 2-instrumen mencapai 3.0 (GM-1), apalagi 4.0 (GM-1b). Angka 1.6562 di atas mengonfirmasi ini secara empiris: **hasil ukur, bukan kejutan** -- sudah diprediksi secara aljabar sebelum data dilihat.


## Verdict gerbang mati

- **GM-1 (K_eff >= 3.0): GAGAL (K_eff terukur = 1.6562)**
